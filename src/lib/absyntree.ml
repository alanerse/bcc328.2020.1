...
let rec tree_of_exp exp =
  ...
  | CondExp (ifExp, thenExp, elseExp) -> mktr (sprintf "CondExp if") [tree_of_lexp ifExp; tree_of_lexp thenExp; tree_of_lexp elseExp]
  | IdExp x                           -> mktr (sprintf "IdExp %s" (name x)) []
  | DecExp (id, x, y)                 -> mktr (sprintf "DecExp %s" (name id)) [tree_of_lexp x; tree_of_lexp y]
  | FuncCallExp (id, x)               -> mktr (sprintf "FuncCallExp %s" (name id)) (List.map tree_of_lexp x)
